...
async def demo_basic_crawl():
    ...
    async with AsyncWebCrawler(config = BrowserConfig(
        ...
        results: List[CrawlResult] = await crawler.arun(
            url="https://www.facebook.com/Entretient.V.Beck/about",
            config=CrawlerRunConfig(
                js_code="""document.querySelector('[aria-label="Close"]')?.click();""",
            )
        )
        ...
        open (f"{__cur_dir__}/tmp/facebook.md", "w").write(results[0].markdown.raw_markdown)
